...
from finetune_portuguese import add_model_arguments, get_model, get_params
from tokenizer import get_tokenizer, Tokenizer
from decoding import DecodingOptions, decode
from post_process import process_text, portuguese_process_text, portuguese_process_text_large
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        if params.use_lora or params.fullfinetune:
            for result in decode_result:
                if params.portuguese_decoding:
                    hyps.append(portuguese_process_text_large(result.text))
                else:
        ...
        for cut_id, hyp, ref_text in zip(cut_ids, hyps, texts):
            if params.portuguese_decoding:
                ref_text = portuguese_process_text_large(ref_text)
